"""Filters generated by field type. Empty filter = unrestricted."""
...
def _widget(df, f: Field, key):
    ...
    if f.kind in ("categorical", "multi"):
        return st.multiselect(label, _options(df, f), key=key, placeholder="All")
    if f.kind == "numeric":
...
def render_filters(dataset: Dataset, preset: list) -> tuple:
    """Renders filters in the sidebar and returns (filtered_df, active_filters_description)."""
    df = dataset.df
