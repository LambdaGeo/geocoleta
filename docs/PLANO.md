...
Fases 0–5 implementadas; 9 testes passando (`cd myframework && python -m pytest -q tests`).
Como usar: veja `myframework/README.md`. Diferenças em relação ao plano original:

- Páginas ficam em `views/` (não `pages/`: o Streamlit transforma `pages/` ao lado do app em navegação automática).
- "Por seção" virou a página **Perguntas** (abas por grupo do form ou `secoes` da config, busca e "cruzar com").
- Tokens ficam em memória no processo (não mais em `token.json`).
- Mapa renderizado como HTML estático (`components.html`) com tiles OpenStreetMap (CartoDB passou a exigir chave).
- Config ganhou `tipos:` para tratar texto livre como categoria (ex.: bairro); grafias que só diferem
  em maiúsculas/espaços são unificadas.

Pendente: itens desmarcados abaixo (limpeza das pastas antigas e migração de `pags/`), além dos riscos no fim.
...
- [x] Garantir `token.json` e `.env` no `.gitignore`.
- [x] Remover caminhos absolutos (`/home/sergio/dev/...`) de `datasources/dados_json.py` e `datasources/epicollect.py`; usar caminhos relativos à raiz do projeto (`Path(__file__).resolve().parents[N]`).
- [x] `main.py`: parar de importar `api.py` (ele faz requisições HTTP na importação).
...
- [x] `core/model.py`:
  ...
  ```
- [x] `core/schema.py` → `parse_form(schema_json) -> list[Field]`:
...
- [x] `reconcile(fields, df_columns)`: compara colunas esperadas × reais e retorna `(ok, faltando, sobrando)`. Se houver divergência, tentar casar por label como fallback e **mostrar aviso** no dashboard (nunca renomear por posição).
- [x] `tests/test_schema.py` com `pesquisaresiduos__diagnostico__form.json` + `dados.json`: todas as colunas de pergunta do `dados.json` precisam casar com um `Field`.
- [x] Substituir `extrair_questoes` e o `df.columns = colunas` em `datasources/dados_json.py` por `parse_form` + `reconcile`.
...
- [x] `""` → `NaN` em todas as colunas.
- [x] `location` (dict) → colunas numéricas `<col>__lat`, `<col>__lon` (+ accuracy opcional).
- [x] `integer`/`decimal` → `pd.to_numeric(errors="coerce")`.
- [x] `date`, `created_at`, `uploaded_at` → `pd.to_datetime`.
- [x] `radio`/`dropdown` → `pd.Categorical(categories=field.options, ordered=True)` (mantém a ordem do form nos gráficos).
- [x] `checkbox` → manter como lista (usar `explode` na hora de contar).
- [x] `html.unescape` nos labels.
...
- [x] `DataSource.load() -> Dataset` (dados **e** schema).
- [x] Registry: `@datasource` registra a **classe**; instanciar só ao usar.
- [x] Auth: reaproveitar `epicollect_auth.py`, mas sem `initialize_tokens()` global; token em cache (`st.cache_resource` ou arquivo em pasta ignorada).
- [x] Schema via API: `GET https://five.epicollect.net/api/export/project/{slug}` → `parse_form` (manter fallback para arquivo local).
- [x] Entradas com **paginação**: `.../api/export/entries/{slug}?form_ref=...&per_page=1000&page=N`, seguir `links.next` / `meta.last_page` até o fim (evita o erro `ec5_335`).
- [x] `@st.cache_data(ttl=300)` no carregamento + botão "Atualizar dados" que limpa o cache.
- [x] Mensagens de erro claras (credencial ausente, 401, limite, rede).
...
- [x] Formato:
  ...
  ```
- [x] `load_config(path)`: YAML + expansão de `${VAR}` do `.env`; validar que aliases existem no schema.
- [x] Script utilitário `python -m myframework.tools.listar_campos <config>`: imprime `ref | coluna | tipo | label` para facilitar escrever os apelidos.
- [x] `app.py`: `streamlit run app.py -- --config projetos/residuos.yaml` (ou variável `DASH_CONFIG`).
...
- [x] `render_filters(dataset, campos)` e `apply_filters` baseados em `Field.type` (não em `isinstance` do valor).
- [x] `render_chart(dataset, field, por=None)`: um gráfico por campo; `por` = cruzamento com outro campo categórico.
- [x] Mapa: usar colunas `__lat/__lon` da Fase 2; centralizar na média; ignorar NaN.

Páginas (`pages/`):
- [x] **Visão geral**: KPIs (total de entrevistas, última coleta, coletores) + coletas por dia + mapa.
- [x] **Por seção**: um expander por `group` do form, com gráfico de cada pergunta automaticamente.
- [x] **Destaques**: gráficos definidos em `destaques` no YAML.
- [x] **Dados**: tabela filtrada + download CSV/Excel.
...
- [x] README do framework: como criar um projeto novo (YAML + `.env` + `listar_campos`).
...
- Perguntas dentro de `branch` saem em outro endpoint (`branch_ref`) — tratar depois, se o form usar (hoje o branch é contado no índice e ignorado).
- Formulários hierárquicos (`hierarchy`, múltiplos forms): começar só com o primeiro form.
- Dados sensíveis (e-mail do coletor em `created_by`): hoje aparecem em "Respostas por coletor"; para esconder, `ignorar: [created_by]` na config.
